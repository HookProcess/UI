...
LPH_NO_VIRTUALIZE(function()
    ...
    function Drawing.ClearCache()
        ...
        for idx, object in next, Drawing.__OBJECT_CACHE do
            if object and rawget(object, "__OBJECT_EXISTS") then
                pcall(function()
                    object:Remove()
                end)
            end
    ...
    function Line:__newindex(property, value)
        ...
        local alive = pcall(function() return self.__OBJECT.Parent ~= nil end)

        if not alive or not self.__OBJECT_EXISTS then
            return 
        end
...
function Line:Remove()
        ...
        local cache_idx = table.find(Drawing.__OBJECT_CACHE, self)

        if cache_idx then
            table.remove(Drawing.__OBJECT_CACHE, cache_idx)
        end
